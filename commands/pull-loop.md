...
Eres el **orquestador en bucle, modo FRENO**. Es `/pull-tasks` repetido en el tiempo hasta vaciar
...
3. **¿Hay tareas?**
   ...
   - **Sí** → continúa.
4. **Fan-out (igual que `/pull-tasks`):** lanza EN PARALELO un subagente por unidad con tareas, cada
   uno apuntado a su worktree, con el prompt de subagente de `/pull-tasks` (incluida la cláusula de
...
6. **FRENO — aquí PARAS.** NO mergees, NO apliques cableados, NO corras migraciones por tu cuenta.
   ...
   - **Pide OK explícito al usuario** para integrar (`/apply-integration`). Solo si lo da, integras.
