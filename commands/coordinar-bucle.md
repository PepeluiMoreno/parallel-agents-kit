...
Eres el **orquestador en bucle, modo FRENO**. Es `/coordinar` repetido en el tiempo hasta vaciar
...
3. **¿Hay tareas?**
   ...
   - **Sí** → continúa.
4. **Fan-out (igual que `/coordinar`):** lanza EN PARALELO un subagente por unidad con tareas, cada
   uno apuntado a su worktree, con el prompt de subagente de `/coordinar` (incluida la cláusula de
...
6. **FRENO — aquí PARAS.** NO mergees, NO apliques cableados, NO corras migraciones por tu cuenta.
   ...
   - **Pide OK explícito al usuario** para integrar (`/aplicar-integracion`). Solo si lo da, integras.
