...
en `/inferir-organizacion`) y **materializas** el equipo de desarrollo virtual: worktrees, identidades, protocolo concreto,
bandejas y comandos de runtime. Eres determinista: solo traduces el contrato a ficheros. Si el
contrato no existe o no valida contra el schema, para y manda ejecutar `/inferir-organizacion` primero.
...
- Copia a `.claude/commands/` los comandos de runtime del kit: `coordinar.md`, `integrar.md`,
  `inbox.md`, `solicitar-integracion.md`, `triaje.md`. (Ya deberían estar si se instaló el kit; si no,
...
- Deja la ficha de dominio sin rellenar (el PO la crea con el usuario en su primer `/productor`),
  o crea el esqueleto desde `templates/producto/_dominio.md.tmpl`.

## Paso 5 — Activar mecanismos opcionales según runtime
- `runtime.loop != "off"`: asegúrate de que `.claude/commands/coordinar-bucle.md` está disponible
  (drena en bucle; en "freno" para antes de mergear/migrar; en "pleno" no pregunta) y documenta el
  uso `/loop /coordinar-bucle` en la guía. En "off" puedes omitirlo.
- `runtime.triaje_desde_subagentes == true`: en el protocolo y en el prompt de subagentes de
  `/coordinar`, habilita que un subagente derive trabajo de otra unidad a su bandeja vía /peticion.
...
generado, y el primer comando a usar (normalmente `/coordinar` en la raíz, o `/inbox` por unidad).
